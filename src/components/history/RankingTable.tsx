        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              <th className="text-left px-3 py-2 text-gray-600 font-medium w-6">#</th>
              <th className="text-left px-3 py-2 text-gray-600 font-medium">選手</th>
              {columns.map(({ key, label }) => (
                <th
                  key={key}
                  className={`px-3 py-2 text-right cursor-pointer font-medium transition-colors ${
            ...
            {sorted.map((s, idx) => {
              ...
              return (
                <tr key={s.playerId} className={idx === 0 ? 'bg-yellow-50/50' : ''}>
                  ...
                  <td className="px-3 py-2.5 text-right text-gray-700">{s.gamesPlayed}</td>
                  <td className="px-3 py-2.5 text-right text-gray-700">{s.wins}</td>
                  <td className="px-3 py-2.5 text-right text-gray-700">
                    {s.wins + s.losses > 0 ? `${Math.round(s.winRate * 100)}%` : '−'}
                  </td>
                  <td className={`px-3 py-2.5 text-right font-medium ${s.pointDiff > 0 ? 'text-green-600' : s.pointDiff < 0 ? 'text-red-500' : 'text-gray-500'}`}>
