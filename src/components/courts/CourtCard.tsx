...
function PlayerName({ playerId, players, playerNumbers, swapMode, selected, isTarget, onClick }: PlayerNameProps) {
  ...
  return (
    ...
    >
      {player.gender === 'male' && <span className="w-2.5 h-2.5 rounded-full bg-sky-400 flex-shrink-0" />}
      {player.gender === 'female' && <span className="w-2.5 h-2.5 rounded-full bg-rose-400 flex-shrink-0" />}
  ...
  return (
    <div className={`bg-white rounded-2xl shadow-sm overflow-hidden ${swapMode ? 'ring-2 ring-blue-400' : ''} ${match.finished ? 'opacity-70' : ''}`}>
      ...
      <div className="flex items-center px-3 py-2.5 gap-2">
        ...
        <span className="shrink-0 text-sm font-bold text-gray-400 tracking-widest">VS</span>
