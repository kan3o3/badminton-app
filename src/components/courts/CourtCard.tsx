...
function PlayerName({ playerId, players, playerNumbers, swapMode, selected, isTarget, onClick }: PlayerNameProps) {
  ...
  return (
    <button
      ...
      disabled={!swapMode}
      className={`inline-flex items-center gap-1.5 rounded-xl px-3 py-2 transition-all text-base font-semibold ${
        swapMode
          ? selected
    ...
    >
      {player.gender === 'male' && <span className="w-3 h-3 rounded-full bg-sky-400 flex-shrink-0" />}
      {player.gender === 'female' && <span className="w-3 h-3 rounded-full bg-rose-400 flex-shrink-0" />}
      {num !== undefined && <span className="text-sm bg-gray-100 text-gray-600 font-bold px-1.5 py-0.5 rounded font-mono">#{num}</span>}
      <span className="text-gray-800">{player.name}</span>
      {player.rank && (
        <span className="text-sm bg-blue-100 text-blue-600 font-bold px-1.5 py-0.5 rounded leading-none">{player.rank}</span>
      )}
    </button>
  )
}
    ...
    <div className={`bg-white rounded-2xl shadow-sm overflow-hidden ${swapMode ? 'ring-2 ring-blue-400' : ''} ${match.finished ? 'opacity-70' : ''}`}>
      ...
      <div className={`${headerBg} px-4 py-3 flex items-center gap-2`}>
        <div className="flex items-center gap-1.5 shrink-0">
          <span className="text-white font-bold text-base">コート {match.courtIndex + 1}</span>
          <button
            onClick={toggleFormat}
            title="形式を切り替え"
            className="text-sm text-white/80 border border-white/30 rounded-md px-2 py-1 hover:bg-white/15 active:bg-white/25 leading-tight"
          >
            {formatLabel}
          </button>
        </div>
        ...
        {match.finished ? (
          <>
            <span className="text-white/70 text-base ml-1">終了</span>
            {scoreLabel && (
              <span className="ml-auto bg-white/20 text-white text-sm font-bold px-2.5 py-1 rounded-lg">{scoreLabel}</span>
            )}
          </>
        ) : (
          <>
            ...
            {!swapMode && (
              <button
                ...
                className="shrink-0 bg-white/20 hover:bg-white/30 active:bg-white/40 text-white text-sm font-bold px-4 py-2 rounded-xl"
              >
                終了
              </button>
            )}
      ...
      <div className="flex items-center px-4 py-3 gap-3">
        ...
        <span className="shrink-0 text-base font-bold text-gray-400 tracking-widest">VS</span>
          ...
          {showScore ? (
            <div className="flex items-center gap-2">
              <input
                ...
                className="w-16 text-center border border-gray-200 rounded-xl py-2.5 text-base font-bold focus:outline-none focus:ring-2 focus:ring-green-400 bg-gray-50"
              />
              ...
              <button
                ...
                className="text-base bg-green-600 text-white px-4 py-2 rounded-xl font-medium"
              >
                保存
              </button>
              <button
                onClick={() => setShowScore(false)}
                className="text-base text-gray-400 hover:text-gray-600 px-2"
              >
                ✕
              </button>
            </div>
            ...
            <button
              ...
              className="text-base text-gray-400 hover:text-gray-600 active:text-gray-800 flex items-center gap-1"
            >
              {scoreLabel
